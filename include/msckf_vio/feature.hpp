...
bool Feature::checkMotion(
  ...
  const StateIDType& last_cam_id = (--observations.end())->first; //这里用了--,莫非end并不是最后一个元素?
// 分别获得first和last两帧的位姿
  ...
  Eigen::Vector3d feature_direction(
      ...
      observations.begin()->second(1), 1.0); //归一化平面上的坐标
  feature_direction = feature_direction / feature_direction.norm();
  feature_direction = first_cam_pose.linear()*feature_direction; //转换到世界坐标系
  ...
  //orthogonal_translation表示世界坐标系中相机中心移动向量与其在特征点在世界坐标系中的方向的投影之差
  //如果相机移动方向平行于该点与世界坐标系原点的连线,则这个值为0.无法进行三角化
  Eigen::Vector3d translation = last_cam_pose.translation() -
    first_cam_pose.translation();  //两帧相机中心的位置差
