...
unsigned runtimes = 0;
double pruneGridFeaturesTime=0,trackFeaturesTime=0,drawFeaturesStereoTime=0;
double publishTime =0,addNewFeaturesTime=0,fullTime = 0,PyramidsTime=0;
double max_tracktime =0,min_tracktime=10.0;
#endif
...
bool ImageProcessor::createRosIO() {
  feature_pub = nh.advertise<CameraMeasurement>(
      "features", 3);                                          //注册发布feature
  tracking_info_pub = nh.advertise<TrackingInfo>(
      "tracking_info", 1);                                     //注册发布tracking_info
  ...
  imu_sub = nh.subscribe("imu", 50,
      &ImageProcessor::imuCallback, this);  //注册订阅imu数据,收到imu信息就放入imu_msg_buffer里
  return true;
}
...
bool ImageProcessor::initialize() {
  ...
  detector_ptr = FastFeatureDetector::create(
      processor_config.fast_threshold);  //fast角点检测阈值,设置为20
...
//创建金字塔,追踪特征点,创建新特征点,去除多余点,发布feature消息,更新当前帧,上一帧的消息
void ImageProcessor::stereoCallback(
  ...
  //金字塔为3层,winSize设为3x3,winSize是padding的大小,用于之后光流的计算
  ros::Time startPyramids_time = ros::Time::now();
  createImagePyramids();
#if record_tracking_time
  PyramidsTime += ((ros::Time::now()-startPyramids_time).toSec());
#endif
  ...
  if (is_first_img) {
    ...
    double  first_frametime = ((ros::Time::now()-start_time).toSec());
    is_first_img = false;
...
#if record_tracking_time
runtimes++;
double track_time =((ros::Time::now()-start_time).toSec());
trackFeaturesTime += track_time;

if (track_time>max_tracktime) {
    max_tracktime = track_time;
    imwrite( "/media/oym/source/oym/MSCKF_VIO/maximage/"+std::to_string(runtimes)+".jpg",cam0_curr_img_ptr->image);
    if(Runtime_File.is_open())  Runtime_File<<(std::to_string(runtimes)+"track_time:")<<track_time<<endl;
}
else if(track_time<min_tracktime){
    min_tracktime = track_time;
    imwrite( "/media/oym/source/oym/MSCKF_VIO/minimage/"+std::to_string(runtimes)+".jpg",cam0_curr_img_ptr->image);
    if(Runtime_File.is_open())  Runtime_File<<(std::to_string(runtimes)+"track_time:")<<track_time<<endl;
}
#endif
    ...
    //    (ros::Time::now()-start_time).toSec());
#if record_tracking_time
    addNewFeaturesTime+=((ros::Time::now()-start_time).toSec());
#endif

    // Add new features into the current image.
    start_time = ros::Time::now();
    //去除多于的点,主要是网格里点超过需要的点,则去除那些存活期较长的点
    pruneGridFeatures();
    //ROS_INFO("Prune grid features: %f",
    //    (ros::Time::now()-start_time).toSec());
#if record_tracking_time
  pruneGridFeaturesTime+=((ros::Time::now()-start_time).toSec());
#endif
    // Draw results.
    start_time = ros::Time::now();
    drawFeaturesStereo();
    ...
    //    (ros::Time::now()-start_time).toSec());
#if record_tracking_time
  drawFeaturesStereoTime+=((ros::Time::now()-start_time).toSec());
#endif
  }
  ...
  //    (ros::Time::now()-start_time).toSec());
#if record_tracking_time
  publishTime+=((ros::Time::now()-start_time).toSec());
#endif
...
#if record_tracking_time
  fullTime += ((ros::Time::now()-beginTime).toSec());
  if(runtimes ==2460){
      Runtime_File<<"================================"<<endl;
      Runtime_File<<"publishTime:"<<publishTime/runtimes<<endl;
      Runtime_File<<"addNewFeaturesTime:"<<addNewFeaturesTime/runtimes<<endl;
      Runtime_File<<"pruneGridFeaturesTime:"<<pruneGridFeaturesTime/runtimes<<endl;
      Runtime_File<<"trackFeaturesTime:"<<trackFeaturesTime/runtimes<<endl;
      Runtime_File<<"drawFeaturesStereoTime:"<<drawFeaturesStereoTime/runtimes<<endl;
      Runtime_File<<"PyramidsTime:"<<PyramidsTime/runtimes<<endl;
      Runtime_File<<"fullTime:"<<fullTime/runtimes<<endl;
      Runtime_File<<"OtherTime:"<<(fullTime-publishTime-addNewFeaturesTime-pruneGridFeaturesTime-trackFeaturesTime-drawFeaturesStereoTime-PyramidsTime)/runtimes<<endl;
      Runtime_File.close();
  }

  if(runtimes %10 == 0) {
      printf("frontend run times: %u\n",
          runtimes);
  }
#endif
...
void ImageProcessor::createImagePyramids() {
  const Mat& curr_cam0_img = cam0_curr_img_ptr->image;  // Group the features into grids:将匹配好的特征点放入划分的网格中
...
//step2:KLT双目匹配
//step3:匹配点分块
//step4:每个方块里选取response较大的几个角点
//经过这几步,使得提取的特征点尽可能均匀分布,且角点响应强度好
void ImageProcessor::initializeFirstFrame() {
  // Size of each grid. grid_row,grid_col均设置为4,要对图像分成4x4块
  ...
  // Detect new features on the frist image. 在原图提取角点
  vector<KeyPoint> new_features(0);
  detector_ptr->detect(img, new_features);

  // Find the stereo matched points for the newly
  // detected features.双目匹配,也是用的LKT光流?
  ...
  for (int i = 0; i < inlier_markers.size(); ++i) {
    ...
    response_inliers.push_back(new_features[i].response);  //response是有关特征点强度的变量
  }

  // Group the features into grids:将匹配好的特征点放入划分的网格中
  ...
  // Sort the new features in each grid based on its response基于response进行排序.
  for (auto& item : grid_new_features)
    std::sort(item.second.begin(), item.second.end(),
        &ImageProcessor::featureCompareByResponse);

  // Collect new features within each grid with high response在每个网格里提取一定数量response较高的特征点.
  for (int code = 0; code <
      processor_config.grid_row*processor_config.grid_col; ++code) {
    ...
    for (int k = 0; k < processor_config.grid_min_feature_num &&  //为何使用grid_min_feature_num?
...
//特征点trcaking
//step1:imu积分,得到从上一帧到当前帧的旋转矩阵
//step2:利用IMU角速度积分得到的角度,预测上一帧cam0中的特征点在当前cam0中的位置
//step3:KLT追踪特征点,并去除界外的点以及未被跟踪上的点栓
//step4:根据当前和之前的双目去除外点,这个步骤又细分为3步,代码中有注释
//    去除无法双目匹配的点;ransac选取最合适的点.
//step5:经过筛选的当前帧特征点放入curr_features_ptr对应的网格里,其id是被追踪特征点的id,可用于后面的匹配
...
//step3
  ...
  ROS_INFO_THROTTLE(0.5,
      "\033[0;32m runtimes: %d; track: %d; match: %d; ransac: %d/%d=%f\033[0m",
...
//双目匹配:
//首先将cam0的特征点投影到cam1中,得到初始位置,这个投影只考虑相机的内参以及之间的旋转变换
void ImageProcessor::stereoMatch(
  ...
  if(cam1_points.size() == 0) {
    ...
    // rotation from stereo extrinsics  根据双目外参在cam1找到cam0特征点的投影位置,用这个位置进行初始化
    const cv::Matx33d R_cam0_cam1 = R_cam1_imu.t() * R_cam0_imu; //cam1与cam0相对位姿,应该是cam1在cam0下的旋转pose,理想情况下为I?
    vector<cv::Point2f> cam0_points_undistorted;
    undistortPoints(cam0_points, cam0_intrinsics, cam0_distortion_model,
                    cam0_distortion_coeffs, cam0_points_undistorted,
                    R_cam0_cam1);  //  特征点去畸变,并进行旋转变换
    cam1_points = distortPoints(cam0_points_undistorted, cam1_intrinsics,
                                cam1_distortion_model, cam1_distortion_coeffs); //近似到cam1上的像素点
  }

  // Track features using LK optical flow method.使用LK光流追踪特征点,用的opencv函数
  ...
  const cv::Vec3d t_cam0_cam1 = R_cam1_imu.t() * (t_cam0_imu-t_cam1_imu);  //右乘联体左乘基,此处以imu作为基坐标系,故左乘
  // Compute the essential matrix.  根据相机相对pose求其本质矩阵
  ...
  // essential matrix. 用本质矩阵剔除误匹配点
  //step1: 特征点去畸变,得到归一化坐标
  ...
  undistortPoints(
      ...
      cam1_distortion_coeffs, cam1_points_undistorted); //去畸变得到归一化坐标
...
//检测cam0已经追踪的特征点之外的新的fast特征点,在cam1中寻找匹配点,并且网格中添加response较大的特征点
...
//如果网格里特征点太多,则去除lifetime较小的特征点,优先保留一直在被跟踪的特征点
...
void ImageProcessor::undistortPoints(
    ...
    const cv::Matx33d &rectification_matrix, //rectification_matrix是cam1相对cam0的旋转位姿?
  ...
  const cv::Matx33d K_new(
      ...
      0.0, 0.0, 1.0);  //cam1内参,new_intrinsics初始化为[1,1,0,0]

  if (distortion_model == "radtan") {
    cv::undistortPoints(pts_in, pts_out, K, distortion_coeffs,
                        rectification_matrix, K_new);  //特征点去畸变得到归一化的坐标,rectification_matrix是相对旋转变换
...
vector<cv::Point2f> ImageProcessor::distortPoints(
  ...
  if (distortion_model == "radtan") {
    ...
    cv::convertPointsToHomogeneous(pts_in, homogenous_pts);  //坐标末尾添"1",变成齐次做标
    cv::projectPoints(homogenous_pts, cv::Vec3d::zeros(), cv::Vec3d::zeros(), K,
                      distortion_coeffs, pts_out);  //将其次坐标投影到像素坐标
  } else if (distortion_model == "equidistant") {
...
//IMU积分
//step1: 以上一帧图像前0.01sbegin_iter到后0.005send_iter之间的角速度平均值作为当前帧与上一帧之间的角速度
//step2: 上述平均角速度转换到cam坐标系
//step3: 角速度乘以两帧之间的时间得到角度,利用Rodrigues转化成旋转矩阵;
//step4:删除imu数组容器里从开始到end_iter之间的数据
...
void ImageProcessor::integrateImuData(
  ...
  while (begin_iter != imu_msg_buffer.end()) {
    if ((begin_iter->header.stamp-
          cam0_prev_img_ptr->header.stamp).toSec() < -0.01) //imu时间戳距离前一帧小于0.01s?
      ++begin_iter;
    else
      break;
  }
...
//两点ransac算法,详解见知乎:VIO/VSLAM理论与实践--->MSCKF那些事(三)算法详解1:前端特征跟踪
//step1:特征点去畸变,并得到归一化平面上的坐标
//step2: Compenstate the points in the previous image with the relative rotation.
//step3:对step1中的特征点进行Normalize
//step4:去除step3中位置差异较大的匹配关系,如果发现运动退化(纯旋转或者一点很小),则返回
//step5:ransac任选两对匹配点(第一个点有左乘R),计算出相对位姿t,最优的t(inlier数最多的t)里的inlier就是最终的inlier,算出的t并没使用
//??imu提供的R有那么可靠吗?
void ImageProcessor::twoPointRansac(
  ...
  // step1:Undistort all the points.
  ...
  //step2: Compenstate the points in the previous image with
  ...
  //step3: Normalize the points to gain numerical stability.
  ...
  for (int iter_idx = 0; iter_idx < iter_num; ++iter_idx) {
    ...
    coeff_l1_norm[0] = coeff_tx.lpNorm<1>();  //1范数,也就是绝对值之和
    coeff_l1_norm[1] = coeff_ty.lpNorm<1>();
    coeff_l1_norm[2] = coeff_tz.lpNorm<1>();
    int base_indicator = min_element(coeff_l1_norm.begin(),
        coeff_l1_norm.end())-coeff_l1_norm.begin();  //范数最小的索引
    ...
    for (int i = 0; i < error.rows(); ++i) {
      if (inlier_markers[i] == 0) continue;
      //norm_pixel_unit=2/(fx+fy),inlier_error = processor_config.ransac_threshold = 3
