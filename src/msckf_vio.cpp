...
Vector3d IMUState::gravity = Vector3d(0, 0, -GRAVITY_ACCELERATION); //GRAVITY_ACCELERATION=9.81
Isometry3d IMUState::T_imu_body = Isometry3d::Identity(); //imu系就是body系
...
#if record_runtime
double imuprocess_time=0;
double augmentation_time=0;
double removeLostfeature_time =0;
double pruncam_time =0;
double full_time =0;
double observations_time=0;
double publishtime =0;
bool already_recorded =0;
ofstream Runtime_File, Pruncam_File;
#endif
...
MsckfVio::MsckfVio(ros::NodeHandle& pnh):
  is_gravity_set(false),  //没有设置重力
  is_first_img(true),     //是第一幅图像
  nh(pnh) {               //node节点赋值?
  return;
}
...
bool MsckfVio::loadParameters() {
  ...
  nh.param<double>("translation_threshold", translation_threshold, 0.4);    //平移阈值设为0.4,用于决定关键帧
  nh.param<double>("tracking_rate_threshold", tracking_rate_threshold, 0.5); //跟随速度阈值设为0.5,用于决定关键帧

  // Feature optimization parameters
  nh.param<double>("feature/config/translation_threshold",
      Feature::optimization_config.translation_threshold, 0.2);   //3D点位置初始化值设为0.2

  // Noise related parameters
  nh.param<double>("noise/gyro", IMUState::gyro_noise, 0.001);    //IMU陀螺仪噪声
  nh.param<double>("noise/acc", IMUState::acc_noise, 0.01);       //加速度计噪声
  nh.param<double>("noise/gyro_bias", IMUState::gyro_bias_noise, 0.001); //陀螺仪bias噪声
  nh.param<double>("noise/acc_bias", IMUState::acc_bias_noise, 0.01);    //加速度计bias噪声
  nh.param<double>("noise/feature", Feature::observation_noise, 0.01);   //归一化特征点观测噪声,是百分比吗?

  // Use variance instead of standard deviation. 方差是标准差的平方
  ...
  // Set the initial IMU state. 设置IMU初始值
  ...
  // there should be nontrivial uncertainty. 这些是测量方差还是实际状态的方差?
  ...
  IMUState::T_imu_body =
    utils::getTransformEigen(nh, "T_imu_body").inverse();  //获得IMU与机体系之间的外参

  // Maximum number of camera states to be stored  最多存储30个相机状态
...
bool MsckfVio::createRosIO() {
  ...
  reset_srv = nh.advertiseService("reset",
      &MsckfVio::resetCallback, this);             //注册重置reset服务

  imu_sub = nh.subscribe("imu", 100,
      &MsckfVio::imuCallback, this);              //注册订阅imu数据信息
  feature_sub = nh.subscribe("features", 40,      //注册订阅特征点信息
...
bool MsckfVio::initialize() {
  if (!loadParameters()) return false;  //加载配置文件中的参数,也有指定初始化参数camID
  ROS_INFO("Finish loading ROS parameters...");
  // Initialize state server 对角矩阵
  ...
  // level 0.95.  初始化卡方分布
  for (int i = 1; i < 100; ++i) {
    boost::math::chi_squared chi_squared_dist(i);  //i是卡方分布的自由度
    chi_squared_test_table[i] =
      boost::math::quantile(chi_squared_dist, 0.05);
  }  
  if (!createRosIO()) return false;  // 注册ros发布和订阅消息的对象
...
//printf("进入featureCallback函数的次数: %u",camID);
...
#if show_state
  if( OutFile.is_open()){
   OutFile<<"这是第"<<camID<<"次进入FeatureCallback函数;扩增之前cam数:"<<state_server.cam_states.size()<<"其ID分别是"<<endl;
   auto cam_state_iter = state_server.cam_states.begin();
   for (int i = 0; i < state_server.cam_states.size();
       ++i, ++cam_state_iter) {
       OutFile<<cam_state_iter->first<<" ";
   }
   OutFile<<endl;
  }
#endif

  // Augment the state vector.得到相机相机的位姿和方差
  ...
  if( OutFile.is_open()){
   ...
   OutFile<<"扩增之后最新一帧图像由IMU计算得到的cam位姿为Q:"<<endl<<cam_state.orientation.transpose()<<"位置:"<<cam_state.position.transpose()<<endl;
   OutFile<<"扩增之后cam数:"<<state_server.cam_states.size()<<"其ID分别是"<<endl;
   auto cam_state_iter = state_server.cam_states.begin();
   for (int i = 0; i < state_server.cam_states.size();
       ++i, ++cam_state_iter) {
       OutFile<<cam_state_iter->first<<" ";
  ...
  // map_server每一个元素代表一个地图点,其observations里存着可以看见该帧的图像id以及在该帧图像上的坐标
  start_time = ros::Time::now();
  ...
  //自添函数:打印更新后的最新帧相机位姿
  if(OutFile.is_open()){
   CAMState& cam_state = state_server.cam_states[state_server.imu_state.id];  //当前的cam位姿
   OutFile<<"EKF更新后最新帧位姿为Q:  "<<endl<<cam_state.orientation.transpose()<<"位置:  "<<cam_state.position.transpose()<<endl;
   OutFile<<"EKF状态更新之后cam数:"<<state_server.cam_states.size()<<"其ID分别是"<<endl;
   auto cam_state_iter = state_server.cam_states.begin();
   for (int i = 0; i < state_server.cam_states.size();
       ++i, ++cam_state_iter) {
       OutFile<<cam_state_iter->first<<" ";
  ...
  if(OutFile.is_open()){
   CAMState& cam_state = state_server.cam_states[state_server.imu_state.id];  //当前的cam位姿
   OutFile<<"去掉多余的之后cam之后最新帧位姿为Q:  "<<endl<<cam_state.orientation.transpose()<<"位置:  "<<cam_state.position.transpose()<<endl;
   OutFile<<"去掉多余的之后cam数:"<<state_server.cam_states.size()<<"其ID分别是"<<endl;
   auto cam_state_iter = state_server.cam_states.begin();
   for (int i = 0; i < state_server.cam_states.size();
       ++i, ++cam_state_iter) {
       OutFile<<cam_state_iter->first<<" ";
...
//打印状态信息:
 ...
 if(camID<=2000){
  ...
  removeLostfeature_time =removeLostfeature_time +remove_lost_features_time ;
  pruncam_time =pruncam_time+prune_cam_states_time ;
  full_time =full_time + processing_time;
  publishtime= publishtime + publish_time  ;
  Pruncam_File<<camID<<": "<<prune_cam_states_time<<endl;
 }
 else if(!already_recorded && Runtime_File.is_open()){
     already_recorded =1;
     Runtime_File<<"augmentation_time:  "<<augmentation_time/camID<<endl;
     Runtime_File<<"observations_time:  "<<observations_time/camID<<endl;
     Runtime_File<<"imuprocess_time:  "<<imuprocess_time/camID<<endl;
     Runtime_File<<"removeLostfeature_time:  "<<removeLostfeature_time/camID<<endl;
     Runtime_File<<"pruncam_time:  "<<pruncam_time/camID<<endl;
     Runtime_File<<"publishtime:  "<<publishtime/camID<<endl;
     Runtime_File<<"full_time:  "<<full_time/camID<<endl;
     Runtime_File<<"cam数量:"<<camID<<endl;
     Runtime_File.close();
     Pruncam_File.close();
 }
...
void MsckfVio::batchImuProcessing(const double& time_bound) {
  ...
  for (const auto& imu_msg : imu_msg_buffer) {
    ...
    if (imu_time < state_server.imu_state.time) {  //去掉初始化重力和陀螺仪bias用到的IMU数据
      ++used_imu_msg_cntr;
      continue;
    }

    if (imu_time > time_bound) break;  //选取两帧之间的IMU数据
...
//IMU状态预测以及协方差矩阵,数学推断见知乎转栏 zhuanlan.zhihu.com/p/76794930
void MsckfVio::processModel(const double& time,
    const Vector3d& m_gyro,
    const Vector3d& m_acc) {

  // Remove the bias from the measured gyro and acceleration
  IMUState& imu_state = state_server.imu_state;   //申明引用定义,也可以说是别名,类似指针,对imu_state的操作也是对state_server.imu_state的操作
  ...
  Matrix<double, 21, 21> Phi = Matrix<double, 21, 21>::Identity() +
    Fdt + 0.5*Fdt_square + (1.0/6.0)*Fdt_cube;      //误差状态转移矩阵
  ...
  imu_state.orientation_null = imu_state.orientation;  //null是经过imu预测之后的新状态
...
void MsckfVio::predictNewState(const double& dt,
  ...
  Vector4d& q = state_server.imu_state.orientation;//申明一个引用,q是后者的别名,对q的操作就是对后者的操作
  ...
  Matrix3d dR_dt_transpose = quaternionToRotation(dq_dt).transpose();  //进行转置:位姿q代表的是从global到IMU的的旋转矩阵
  Matrix3d dR_dt2_transpose = quaternionToRotation(dq_dt2).transpose(); //而积分计算的时候需要将测量值从IMU转到global系下
...
void MsckfVio::stateAugmentation(const double& time) {
  ...
  CAMState& cam_state = state_server.cam_states[
    state_server.imu_state.id];  //当前的cam位姿
  ...
  // Resize the state covariance matrix.新加入一幅图像,所以要扩6
  size_t old_rows = state_server.state_cov.rows();
  size_t old_cols = state_server.state_cov.cols();
  state_server.state_cov.conservativeResize(old_rows+6, old_cols+6);

  // Rename some matrix blocks for convenience.P11---IMU的协方差矩阵 P12---IMU与相机pose之间的协方差矩阵
  ...
  state_server.state_cov.block(old_rows, 0, 6, old_cols) << J*P11, J*P12;  //当前帧与IMU的协方差
  state_server.state_cov.block(0, old_cols, old_rows, 6) =
    state_server.state_cov.block(old_rows, 0, 6, old_cols).transpose();    //当前帧与IMU的协方差
  state_server.state_cov.block<6, 6>(old_rows, old_cols) =
    J * P11 * J.transpose(); //加入新的相机位姿协方差矩阵

  // Fix the covariance to be symmetric变成对称矩阵
  MatrixXd state_cov_fixed = (state_server.state_cov +
      state_server.state_cov.transpose()) / 2.0;
  state_server.state_cov = state_cov_fixed;
...
//如果是新的特征点,则将其加入到map_server中
//如果是之前存在的特征点,则将当前帧加入到该点的观测之中
void MsckfVio::addFeatureObservations(
    const CameraMeasurementConstPtr& msg) {

  StateIDType state_id = state_server.imu_state.id;  //每次两帧之间的IMU被处理完后,该id++1,该ID与历史图像帧数量保持一致
...
//计算该特征点的EKF标准观测模型: r_j=H_xj * x,详见msckf-vio论文III-B:Measurement_Model和III-C:Observability Constraint.
//step1:找出有多少帧图像看到该特征点
//step2:计算观测残差r对以及相机pose和特征点位置的雅克比矩阵(H_xj和H_fj)以及观测H残差r
//step3:将r及H_xj投影到H_fj的零空间,最终形成标准的EKF观测模型
void MsckfVio::featureJacobian(
  ...
  for (const auto& cam_id : valid_cam_state_ids) {
    ...
    Matrix<double, 4, 6> H_xi = Matrix<double, 4, 6>::Zero(); //观测残差对相机pose的雅克比
    Matrix<double, 4, 3> H_fi = Matrix<double, 4, 3>::Zero();  //观测残差对特征点三维位置的雅克比
    Vector4d r_i = Vector4d::Zero();
    //得到观测残差对imu和相机pose残差的雅克比矩阵,等同于观测对imu和相机位姿的雅克比矩阵
    ...
    // Stack the Jacobians.
    H_xj.block<4, 6>(stack_cntr, 21+6*cam_state_cntr) = H_xi;  //还不涉及IMU状态,所以索引+21
...
//EKF计算过程:详见msckf-mono论文III-E:EKF Updates Mechanism
void MsckfVio::measurementUpdate(
  ...
  // complexity as in Equation (28), (29).减少计算量,这两个公式是msckf-mono论文里对应的公式
  MatrixXd H_thin;
  VectorXd r_thin;
  ...
  //要求x=B*A.inverse(),等同于解Ax=B,注意参数里有对称矩阵,乍看上去转置混乱,但实际上是正确的.
...
void MsckfVio::removeLostFeatures() {
  ...
  //自己添加的代码,记录状态更新的观测模型涉及的cam
    vector<StateIDType> mearsure_cam;
    vector <StateIDType>::iterator iElement;
...
//step1:
  for (auto iter = map_server.begin();
    ...
    // Pass the features that are still being tracked.在end之前找到imu_state_id,说明这个点在最新帧里有被跟踪到
    if (feature.observations.find(state_server.imu_state.id) !=
        feature.observations.end()) continue;
    //最新帧里没跟踪到,且被观测次数少于3次,这样的点被认为是不合理的点
    ...
    if (!feature.is_initialized) {
        //检查相机的运动,以确保足够的旋转平移变化来对特征点进行三角化
    ...
    jacobian_row_size += 4*feature.observations.size() - 3;  //为何减3?
...
//step2:
  MatrixXd H_x = MatrixXd::Zero(jacobian_row_size,
      21+6*state_server.cam_states.size());        //系统状态imu(21个参数)+cam_pose(6*相机状态个数)
  ...
  for (const auto& feature_id : processed_feature_ids) {
    ...
    for (const auto& measurement : feature.observations){
      ...
      iElement = find(mearsure_cam.begin(),mearsure_cam.end(),measurement.first);
      if(iElement == mearsure_cam.end()) mearsure_cam.push_back(measurement.first);
    }

    MatrixXd H_xj;
    VectorXd r_j;
    //计算该特征点的EKF标准观测模型: r_j=H_xj * x
    featureJacobian(feature.id, cam_state_ids, H_xj, r_j);  //计算出来的H_xj有关IMU部分的值都为0
  ...
  // Remove all processed features from the map.处理过的特征点都是在最新帧里观测不到的点,且下次更新不再需要了
  for (const auto& feature_id : processed_feature_ids)
    map_server.erase(feature_id);

#if show_state
  if(OutFile.is_open()){
   OutFile<<"观测涉及到的camID数量为:"<<mearsure_cam.size()<<"其ID分别是"<<endl;
   for (auto it:mearsure_cam) OutFile<<it<<" ";
   OutFile<<endl;
  }
#endif
  return;
...
//比较最新第2,3帧相较最新第4帧的移动量,如果发现移动量太小,则移除这些帧,否则移除最老的帧
void MsckfVio::findRedundantCamStates(
  ...
  for (int i = 0; i < 4; ++i)
    --key_cam_state_iter;
  auto cam_state_iter = key_cam_state_iter;  //第3最新的帧?
  ++cam_state_iter;
...
//移除多余的相机状态
//step1:选出要移除的两帧图像I1,I2
//step2:map_serve里的特征点除去I1,I2观测,如果不能被滑动窗口中的观测(此时还没移除I1,I2)初始化,则去除该特征点
//step3:求状态偏差对要移除的cam位姿偏差的雅克比矩阵以及相应观测偏差,类似之前的方法,EKF更新位姿
//step4:移除I1,I2以及对应协方差
void MsckfVio::pruneCamStateBuffer() {

  if (state_server.cam_states.size() < max_cam_state_size)  //max_cam_state_size=30,但在msckf_vio_euroc.launch里设置为20
    return;

  //step1: Find two camera states to be removed.
  ...
  for (auto& item : map_server) {
    ...
    // with this feature.  计算特征点的观测里有几个要被remove掉
  ...
  for (auto& item : map_server) {
    ...
    if (involved_cam_state_ids.size() == 0) continue;
//要移除的cam观测残差对特征点和相机位姿误差和特征点位置误差求标准EKF观测方程
    MatrixXd H_xj;
    VectorXd r_j;
  ...
  measurementUpdate(H_x, r);  //上述步骤里并没有观测残差对其他帧的雅克比以及其他帧的残差,所以这一步应该只会更新要移除的帧的位姿以及相关状态方差,对其他帧也可能有影响
...
//如果系统的位置标准差超过一定的阈值,就对系统进行重置
//重置包括:state_server内的相机清零,map_server的特征点清零,IMU状态的方差重新初始化,其中pose和position的方差设为0,但IMU的位置,速度,pose并没变
void MsckfVio::onlineReset() {
  ...
  if (position_x_std < position_std_threshold &&
      ...
      position_z_std < position_std_threshold) return;  //position_std_threshold系统初始化设置为8,单位是cm还是?
...
//发布的消息有:里程计信息:IMU位姿T(旋转+位置)以及协方差,IMU坐标系速度及协方差以及初始化了的特征点三维位置
